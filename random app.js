...
function next(){
    ...
    //    quizOver(); 
        ...
        // preloadVideo();
        // hidingPreloadedVideo();
        getNewQuestion();
       
    }
}
// function hidingPreloadedVideo(){
...
function preloadVideo(){
    ...
    const preloaded=Videosrc2[selectedVideo];
    const preloaded2=Videosrc[selectedVideo];
    if(correctAnswer < 8 && attempt >0 ){
        // resultBox.classList.remove("hide");
        createReaction2(preloaded,selectedVideo);
    }
    else if(correctAnswer >= 8){
        createReaction(preloaded2,selectedVideo);
    }
    else if(attempt === 0){
        resultBox.classList.remove("hide");
    }
 ...
 function createReaction2(preloaded,selectedVideo){
    if(questionCounter === quizLimit){
        // const selectedVideo=Math.floor(Math.random()*Videosrc2.length);
        const times=duration[selectedVideo];
        videos.innerHTML=`<video id="myVideo"><source src="${preloaded}" type="video/mp4"></video>`;
...
function createReaction(preloaded2,selectedVideo){
    const selectedVideo2=Math.floor(Math.random()*Videosrc.length);
    const yeroo=duration1[selectedVideo];
    videos.innerHTML=`<video id="myVideo" ><source src="${preloaded2}" type="video/mp4"></video>`;
    videos.classList.remove("hide");
    console.log(videos);
    playVideo(yeroo);
}
...
function lazyVideo(){
    // const selectedVideo=Math.floor(Math.random()* Videosrc2.length);
     for( i=0; i < Videosrc2.length; i++) {
        videos.innerHTML=`<video id="myVideo" **preload="auto"**><source src="${Videosrc2[i]}" type="video/mp4"></video>`;
    // const reaction=document.getElementById("myVideo");
    // videos.appendChild(reaction);
    // reaction.play(); 
        // const preloaded=Videosrc2[selectedVideo];
